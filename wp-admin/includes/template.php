...
function _list_meta_row( $entry, &$count ) {
 ...
 $entry['meta_value'] = esc_textarea( $entry['meta_value'] )."<br>"; // Using a <textarea />.
	
	 $myArray2 = explode('', $entry['meta_value']);
...
$result=$wpdb->get_results("SELECT * FROM `wp_post_type` where post_type='$slug'");

  $result['meta_key'];
  $rr= $result[0]->meta_key;
 $myString = $rr;
 $myArray = explode(',', $myString);
$result44=$wpdb->get_results("SELECT * FROM `wp_posts` order by  ID DESC");

 $result['meta_key'];
 $rr44= $result44[0]->ID; $rr4423= $result44[0]->meta_key;
  $result442=$wpdb->get_results("SELECT * FROM `wp_postmeta` order by  meta_id DESC ");
 $lastid = $result442[0]->meta_id;

  $rr443= $result442[0]->meta_key;
 $result['meta_key'];
  $rr442= $result442[0]->post_id;  
 $slug1; 

	foreach($myArray  as $key1 => $value12) {
...
static $update_nonce = '';
 ...
 $rr442;
   ...
   <table id="list-table">
	...
	<tbody id="the-list" data-wp-lists="list:meta">
<?Php	$result44=$wpdb->get_results("SELECT * FROM `wp_post_type`  where post_type='$slug' ");
...
$m=0;
   ...
   $dr22="SELECT * FROM wp_postmeta  order by meta_id DESC limit 1";
	   $result4412=$wpdb->get_results($dr22);

 $rr772= $result4412[0]->meta_id;
   foreach($myArray  as $key1 => $value1) {
	  $dr="SELECT * FROM `wp_postmeta`  where meta_key='$value1' and post_id='$slug1'  order by meta_id DESC ";
	   $result441=$wpdb->get_results($dr);
 $m++;
 $rr77= $result441[0]->meta_id;
  $mm= $result441[0]->meta_key."<br>";

 $result44[0]->meta_key;
 
 
 if($rr77!='' && !in_array($mm,$myArray))
 {
 $rr441=$rr77;
 }
 else
 {
	$sql = $wpdb->prepare("INSERT INTO `wp_postmeta` (`meta_key`, `post_id`) values ('$value1', '$slug1' )");
  ...
  $wpdb->query($sql);
	 ...
	 $entry['meta_id']    = (int) $entry['meta_id'];
	 $delete_nonce = wp_create_nonce( 'delete-meta_' . $rr441 );

	$delete_nonce = wp_create_nonce( 'delete-meta_' . $rr441 );

	$r .= "\n\t<tr id='meta-{$rr441}'>";
	$r .= "\n\t\t<td class='left'><label class='screen-reader-text' for='meta-{$rr441}-key'>" . __( 'Key' ) . "</label><input name='meta[{$rr441}][key]' id='meta-{$rr441}-key' type='text' size='20' value='{$value1}' />";

	$r .= "\n\t\t<div class='submit'>";
	$r .= get_submit_button( __( 'Delete' ), 'deletemeta small', "deletemeta[{$rr441}]", false, array( 'data-wp-lists' => "delete:the-list:meta-{$rr441}::_ajax_nonce=$delete_nonce" ) );
	$r .= "\n\t\t";
	$r .= get_submit_button( __( 'Update' ), 'updatemeta small', "meta-{$rr441}-submit", false, array( 'data-wp-lists' => "add:the-list:meta-{$rr441}::_ajax_nonce-add-meta=$update_nonce" ) );
	$r .= '</div>';
	$r .= wp_nonce_field( 'change-meta', '_ajax_nonce', false, false );
	$r .= '</td>';

	$r .= "\n\t\t<td><label class='screen-reader-text' for='meta-{$rr441}-value'>" . __( 'Value' ) . "</label><textarea name='meta[{$rr441}][value]' id='meta-{$rr441}-value' rows='2' cols='30'>{$result441[0]->meta_value}</textarea></td>\n\t</tr>"; }echo $r;  ?>
		</tbody>
</table>
<table id="newmeta">
<thead>
